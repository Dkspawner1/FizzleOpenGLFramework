...
void Renderer::DrawQuad(const glm::vec2 &position, const glm::vec2 &size,
  ...
  float vertices[] = {
      ...
      // Second triangle
      position.x + size.x, position.y, color.r, color.g, color.b, color.a,
      position.x + size.x, position.y + size.y, color.r, color.g, color.b,
      color.a, position.x, position.y + size.y, color.r, color.g, color.b,
      color.a};
...
void Renderer::checkShaderCompileErrors(GLuint shader,
